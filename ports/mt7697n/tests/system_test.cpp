...
extern volatile uint32_t mt7697_boot_record[4];
...
int main() {
  ...
  // SDK whole-chip reboot loses WDT cause, retains reserved SRAM.
  mt7697::capture_reset_reason();
  assert(mt7697::reset_reason() == ResetReason::Software);
  mt7697::capture_reset_reason();
  assert(mt7697::reset_reason() == ResetReason::Unknown); // Intent consumed.
  mt7697::prepare_restart();
  for (auto& word : mt7697_boot_record) word = 0; // Supply lost even with pending intent.
  mt7697::capture_reset_reason();
  assert(mt7697::reset_reason() == ResetReason::PowerOn);
  ...
  assert(mt7697::reset_reason() == ResetReason::Unknown);
  mt7697::prepare_restart();
  mt7697_boot_record[3] ^= 1; // Torn/corrupt intent must not be accepted as warm.
  mt7697::capture_reset_reason();
  assert(mt7697::reset_reason() == ResetReason::PowerOn);
  assert(mt7697::free_heap() == 54321);
  assert(mt7697::minimum_free_heap() == 12345);
  assert(mt7697::stack_low_water_bytes() == 492);
  puts("System tests passed: cold boot, consumed warm intent, power loss, watchdog, invalid marker, heap and stack units.");
}
