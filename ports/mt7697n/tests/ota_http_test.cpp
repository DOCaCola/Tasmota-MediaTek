...
int main(int argc, char** argv) {
  ...
  OtaUrl url;
  assert(parse_ota_url("http://10.9.8.2:8000/test.bin?x=1", url));
  ...
  for (size_t fragment : {1, 2, 155, 156, 157, 701, 1024}) {
    ...
    assert(d.start("http://10.9.8.2:8000/test.bin", 0));
    assert(!d.start("http://10.9.8.2/test.bin", 0));
    run(d);
  ...
  for (auto headers : {"Transfer-Encoding: chunked\r\n", "Content-Encoding: gzip\r\n",
    ...
    assert(d.start("http://10.9.8.2/a",0)); run(d);
  ...
  for (unsigned mode = 0; mode < 4; ++mode) {
    ...
    assert(dd.start("http://10.9.8.2/a",UINT32_MAX-100));
  ...
  for (const auto& h : {std::string(300,'x') + "\r\n",
    ...
    assert(dd.start("http://10.9.8.2/a",0)); run(dd);
  ...
  // Progress does not extend the five-minute absolute deadline.
  {
    ...
    assert(dd.start("http://10.9.8.2/a",0));
