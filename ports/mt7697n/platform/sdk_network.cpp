...
bool station_start(const char* ssid, const char* password) {
  started = false;
  init_global_connsys();
  // Clear both radio association and the previous lease before configuring a
  // new trial. Otherwise a cached address can falsely validate a new password.
  if (wifi_config_set_radio(0) < 0 || !interface_request(false, true)) return false;
  ...
  if (wifi_config_set_ssid(WIFI_PORT_STA,
      ...
      wifi_config_set_radio(1) < 0 || wifi_config_reload_setting() < 0) return false;
  started = true;
  return true;
}
bool station_stop() {
  ...
  const bool radio_stopped = wifi_config_set_radio(0) >= 0;
  return interface_request(false, true) && radio_stopped;
}
bool station_online() {
